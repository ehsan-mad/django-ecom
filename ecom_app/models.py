...
class UserPermission(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="employee_user_for_permission"
    )
    menu = models.ForeignKey(
        MenuList, on_delete=models.CASCADE, related_name="menu_for_permission"
    )
...
class ProductMainCategory(models.Model):
    ...
    cat_slug = models.SlugField(max_length=100, unique=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.cat_slug and self.main_cat_name:
            ...
            slug = base_slug
            num = 1
            while ProductMainCategory.objects.filter(cat_slug=slug).exists():
...
class ProductSubCategory(models.Model):
    main_category = models.ForeignKey(
        ProductMainCategory, on_delete=models.CASCADE, related_name="sub_category"
    )
    sub_cat_name = models.CharField(max_length=100, unique=True)
    sub_cat_slug = models.SlugField(max_length=100, unique=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.sub_cat_slug and self.sub_cat_name:
            ...
            slug = base_slug
            num = 1
            while ProductSubCategory.objects.filter(sub_cat_slug=slug).exists():
                slug = f"{base_slug}-{num}"
                num += 1
...
class Product(models.Model):
    ...
    discount_price = models.DecimalField(max_digits=5, decimal_places=2, default=0.00 , blank=True, null=True)
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="product_created_by"
    )
    updated_by = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
...
class OrderCart(models.Model):
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name="orders_cart")
    ...
    def total_amount(self):
        value = float(self.product.price) * float(self.quantity)
        return value
